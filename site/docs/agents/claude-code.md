...
Set these in your shell profile (see your [platform guide]({{ site.baseurl }}/#platforms) for the exact file and OS specifics):
...
In a Claude Code session, run `/status`, or simply ask it to explain a function. To confirm the request was routed by NeoSmith, curl `/whoami` with your key (see [Verify Connection](../reference/verify-connection)).

## Troubleshooting

See [Troubleshooting](../reference/troubleshooting). Most common issue: env vars set in `~/.zshrc` aren't seen by GUI apps launched from the Dock on macOS — use `~/.zprofile` or launch the IDE from a terminal.
