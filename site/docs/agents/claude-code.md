...
In a Claude Code session, run `/status`, or simply ask it to explain a function. To confirm the request was routed by NeoSmith, curl `/whoami` with your key (see [verify-connection.md](../reference/verify-connection)).

## Troubleshooting

See [reference/troubleshooting.md](../reference/troubleshooting). Most common issue: env vars set in `~/.zshrc` aren't seen by GUI apps launched from the Dock on macOS — use `~/.zprofile` or launch the IDE from a terminal.
